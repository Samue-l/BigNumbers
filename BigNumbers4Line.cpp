...
// 			   y - 0 or 2 depending on row of 16x4 line display								
...
void BigNumbers::displayLargeInt(int n, byte x, byte y, byte digits, bool leading)
{
...
void BigNumbers::clearLargeNumber(byte x, byte y) // x is column of upper left corner for large character
{
  _lcd->setCursor(x,y);
  _lcd->print("   ");
...
void BigNumbers::displayLargeNumber(byte n, byte x, byte y) // n is number to display, x is column of upper left corner for large character
{
  switch (n)
  {
    case 0:
